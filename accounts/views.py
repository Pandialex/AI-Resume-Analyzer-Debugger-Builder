...
# ---------------- Register (with auto-login and optional welcome email) ----------------
def register(request):
    if request.method == "POST":
        form = UserRegisterForm(request.POST)
        if not form.is_valid():
        ...
        # ✅ OPTIONAL: Send welcome email (not mandatory)
        try:
            send_welcome_email(user)
            messages.success(request, f"Welcome to ARDAA, {user.name}! Check your email for welcome message. 🎉")
        except Exception as e:
...
def forget_password_request(request):
    if request.method == "POST":
        ...
        # For password reset, you might want to keep OTP or use a different method
        messages.info(request, "Password reset feature is currently under maintenance. Please contact support.")
        return redirect("accounts:login")
